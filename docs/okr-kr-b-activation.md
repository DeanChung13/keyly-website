...
>
> **更新（2026-08-19）：現為 19 個，實測確認。** 再新增 `cta_type`、`current_step`、`remaining_quota_bucket`（§10 的 Tier 1 三個）、`page_index`、`prompt_id`（KR-3 tutorial）、`prompt_type`（Tier 2，2026-08-19 註冊，趕在新版上架前完成——GA4 不回溯，晚一天就少一天資料）。
>
...
| **註冊 §10 的三個 Tier 1 自訂維度** | ~~2026-08-24~~ | **2026-08-19 已完成**。`cta_type`、`current_step`、`remaining_quota_bucket` 皆已註冊，實測確認 |
...
| 日期 | 事項 |
|---|---|
| ~~2026-08-24~~ | ~~註冊 `current_step` 自訂維度~~ **本列作廢：已於 2026-08-19 完成，所有前置結案** |
