...
| 目標 | 現值 | 目標值 | 分母 | 停損日 |
|---|---:|---:|---:|---|
| **3a** `first_open` → `paywall_opened` | 17.9% | **32%** | 173 | 2026-10-31 |
| **3b** `paywall_opened` → `subscription_purchased` | 12.9% | **20%** | 31 | 2026-10-31 |
| **KR-4** 存活率（先行指標 D1） | 7.8% | 20% | 51 | 與 KR-1 綁定 |
| **KR-4** 存活率（先行指標 D7） | 2.2% | 10% | 46 | 與 KR-1 綁定 |

**判定門檻設在子閘門，不設在整體毛轉換 1.78% → 5.0% 上。** 整體數字的分子只有 4，在 KR-1 達成前不可能取得可判讀樣本；子閘門的分母是 173 與 31，現有量級下可讀。

**成功條件**：3a ≥ 25% 或 3b ≥ 17%（2026-10-31 判定）。兩者皆未達 → 宣告轉換乘數封頂，代入算式重算 KR-1 需求。
...
| **修 `paywall_opened` 的 `entry_point`**（現 90% `(not set)`，讓 Paywall 分析無法進行） | **2026-08-24** | 未做，需改程式 |
| 補「開始註冊」事件 | **2026-08-24** | 未做，需改程式 |
| 確認五個有定義無觸發點的事件是否漏接 | **2026-08-24** | 未做 |

### 主行動（產出是數字的變化）

| 項目 | 推哪個閘門 | 期限 |
|---|---|---|
| 改首次啟動流程 | 3a：17.9% → 32% | 2026-10-10 |
| 改 paywall 進入時機、方案呈現、試用設計 | 3b：12.9% → 20%（31 → 8 的 74% 落差是漏斗後段最大單一缺口） | 2026-10-10 |
...
| 日期 | 事項 |
|---|---|
| 2026-08-24 | 三項前置到期。逾期即放棄該分析，主行動改為在無 `entry_point` 資料下進行 |
| 2026-10-31 | KR-3 停損：3a ≥ 25% 或 3b ≥ 17% |
