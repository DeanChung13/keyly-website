...
## 9. 網站到底帶來多少下載：測不到，但不是零

**補查日期**：2026-09-09，來源 `App Downloads Detailed` 與 Sales MONTHLY 報表。

### 月首次下載（Sales 報表，`1F` + `3F`）
...
8 月 App Store 曝光 2,566、首次下載 49 → 曝光→下載 **1.9%**，與 [okr.md](../okr.md) `:47` 的 1.81% 基準一致。

### 但下載的來源拆不出來

`App Downloads Detailed` 對每個（來源 × 地區 × 版本 × 月）分桶套用揭露門檻，
**低於 5 的桶整列不出現**。實際回傳的列全部 ≥ 5。

因此 8 月 49 次下載中，`Web referrer` 的部分**沒有出現在報表裡**，
只能推得它 **每桶 < 5**。以 8 月 web referrer 產品頁瀏覽 11 次推算，
下載數是個位數，**佔比不明**。

**不得把「報表沒有這一列」讀成「這條來源是 0」。**

### 反證：這條路徑本身出得了量

2026-04 的 `Web referrer` 首次下載共 **88 次**，`Source Info` 全部是 **`applion.jp`**
（日本的 App 介紹站），地區 JP。

這證明 web referrer 通道本身能產生有意義的下載量——
**目前做不到的是 `keylyapp.com`，不是這個通道**。

### 一條結構性測不到的路徑

在桌機或別的情境看到網站、之後在手機 App Store 搜尋「Keyly」安裝，
會被記為 **App Store search**，不是 Web referrer。

品牌詞 `keyly` 在 GSC 本期有 275 次曝光。這條路徑無法歸因，
**不得假設它很大，也不得假設它是零**。若要量測，需要新的機制（例如網站專用的
App Store 連結加上 campaign 參數），本輪未評估成本。
