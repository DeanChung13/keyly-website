    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-metal-gray/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            <a href={APP_STORE_URL} target="_blank" rel="noopener noreferrer" onClick={() => trackDownload('navbar_en')} className="bg-bg-primary text-white px-5 py-2 rounded-full font-medium hover:bg-bg-secondary transition-colors flex items-center space-x-2">
      ...
      {isOpen && (
        <div className="md:hidden bg-white border-b border-metal-gray/30 px-4 pt-2 pb-4 space-y-2">
          ...
          <a href={APP_STORE_URL} target="_blank" rel="noopener noreferrer" className="block px-3 py-2 text-brand-cyan font-medium hover:bg-metal-white/50 rounded-md" onClick={() => { setIsOpen(false); trackDownload('navbar_mobile_en'); }}>Download</a>
        </div>
      )}
    </nav>
  );
  ...
  return (
    <section id="faq" className="py-24 bg-bg-primary text-white overflow-hidden relative">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="space-y-4">
          {faqs.map((faq, index) => (
            <div key={index} className="bg-bg-secondary rounded-2xl border border-white/10 overflow-hidden">
              <button className="w-full px-6 py-5 flex items-center justify-between text-left focus:outline-none" onClick={() => setOpenIndex(openIndex === index ? null : index)}>
              ...
              <AnimatePresence initial={false}>
                {openIndex === index && (
                  <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} transition={{ duration: 0.3, ease: "easeInOut" }} style={{ overflow: 'hidden' }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row justify-between items-center">
          ...
          <div className="flex flex-wrap justify-center items-center gap-3 text-sm text-text-secondary">
            <a href="/privacy/en/" className="hover:text-brand-cyan transition-colors">Privacy Policy</a>
            <span className="text-metal-gray/50">|</span>
            <a href="/terms/en/" className="hover:text-brand-cyan transition-colors">Terms of Service</a>
            <span className="text-metal-gray/50">|</span>
            <a href="/subscriptions/en/" className="hover:text-brand-cyan transition-colors">Auto-Renewal Terms</a>
            <span className="text-metal-gray/50">|</span>
            <a href="mailto:support@keylyapp.com" className="hover:text-brand-cyan transition-colors">Support</a>
          </div>
        </div>
