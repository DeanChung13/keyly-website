    ...
    <nav className="fixed top-3 left-0 right-0 z-50 px-3 sm:px-4">
      ...
      {isOpen && (
        <div className="md:hidden mt-2 max-w-6xl mx-auto bg-white/95 backdrop-blur-xl border border-metal-gray/30 rounded-2xl px-4 pt-2 pb-4 space-y-2 shadow-lg">
          ...
          <a href={DOWNLOAD_URL} target="_blank" rel="noopener" className="block px-3 py-2 text-brand-cyan font-medium hover:bg-metal-white/50 rounded-md transition-colors duration-200" onClick={() => { setIsOpen(false); trackDownload('navbar_mobile_en'); }}>Download Now</a>
        </div>
      )}
    </nav>
  );
